...
function AuditPage({ sdk, locale, user }: { sdk: AuthSdk; locale: Locale; user: User }) {
  ...
  return <div className="flex flex-col gap-5">
    <Card><CardHeader><CardTitle>{t.auditTitle}</CardTitle><CardDescription>{t.auditDescription}</CardDescription></CardHeader><CardContent>
      {!data?.length ? <EmptyState icon={<ScrollTextIcon />} title={t.noAudit} description={t.noAuditDescription} /> : <DataTable><Table><TableHeader><TableRow><TableHead>{t.time}</TableHead><TableHead>{t.endpointModel}</TableHead><TableHead>{t.apiKey}</TableHead><TableHead>{t.channel}</TableHead><TableHead>{t.status}</TableHead><TableHead>{t.usage}</TableHead></TableRow></TableHeader><TableBody>{data.map((row) => <TableRow key={row.id}><TableCell className="whitespace-nowrap">{formatTime(row.created_at, locale)}</TableCell><TableCell><div className="flex max-w-56 flex-col gap-0.5"><span className="font-medium">{row.path}</span><code>{row.model || "—"}</code><code title={row.request_id} aria-label={`${t.requestId}: ${row.request_id}`}>{row.request_id.slice(0, 12)}…</code></div></TableCell><TableCell><code>{row.key_prefix}…</code></TableCell><TableCell><code title={row.channel_id}>{row.channel_id?.slice(0, 8) || "—"}</code></TableCell><TableCell><div className="flex max-w-48 flex-col gap-1"><Badge className="w-fit" variant={row.status >= 400 ? "destructive" : "secondary"}>{row.status}</Badge><span className="text-xs tabular-nums text-muted-foreground">{formatLatency(row.latency_ms)}</span>{row.error && <span className="text-xs text-destructive">{row.error}</span>}</div></TableCell><TableCell><div className="flex items-center gap-3 text-xs tabular-nums"><div className="flex flex-col gap-0.5"><span className="text-muted-foreground">{t.input}</span><span>{row.input_tokens.toLocaleString()}</span></div><div className="flex flex-col gap-0.5"><span className="text-muted-foreground">{t.cachedInput}</span><span>{row.cached_tokens.toLocaleString()}</span></div><div className="flex flex-col gap-0.5"><span className="text-muted-foreground">{t.output}</span><span>{row.output_tokens.toLocaleString()}</span></div></div></TableCell></TableRow>)}</TableBody></Table></DataTable>}
    </CardContent></Card>
