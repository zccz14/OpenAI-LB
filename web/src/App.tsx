...
type Locale = "zh" | "en"
type Page = "dashboard" | "channels" | "keys" | "usage" | "audit" | "request-detail" | "users" | "settings"
...
type Audit = { id: string; request_id: string; user_id: string; key_name: string; channel_id?: string; channel_name?: string; path: string; model?: string; status: number; latency_ms: number; input_tokens: number; output_tokens: number; cached_tokens: number; error?: string; created_at: number }
...
const copy = {
  zh: {
    ...
    pageDashboard:"查看渠道容量与当前租户的 24 小时运行摘要。",pageChannels:"注册、刷新、停用并观察 CodeX OAuth 渠道。",pageKeys:"创建和吊销租户调用凭据；密钥仅显示一次。",pageUsage:"按 API Key 核算请求、Token、错误和延迟。",pageAudit:"逐次追踪请求、渠道、结果与用量；诊断内容按配置期限保留。",pageRequestDetail:"查看调用上下文、消息结构与同一亲和链的相邻请求。",pageUsers:"由 root 管理本地授权角色；用户身份仍由 Auth Mini 提供。",pageSettings:"确认身份边界、上游与部署限制。",
    ...
    auditTitle:"逐调用审计",auditDescription:"请求/响应诊断预览保存在 SQLite；不记录 Authorization 或 OAuth 凭据。",noAudit:"暂无审计事件",noAuditDescription:"每次代理调用结束后都会写入基础审计记录。",time:"时间",requestId:"请求 ID",channel:"渠道",endpointModel:"接口 / 模型",latency:"延迟",cachedInput:"缓存输入",details:"详情",requestDetail:"请求详情",backToAudit:"返回审计",auditDetailDescription:"展示已保留的请求与响应诊断预览；敏感凭据不会记录。",requestHeaders:"请求头",requestBody:"请求正文",responseHeaders:"响应头",responseBody:"响应正文",previewTruncated:"预览已截断",auditArchiveUnavailable:"此请求的诊断记录已过期或不可用。",affinitySource:"亲和来源",affinityRequestId:"亲和请求 ID",affinityHash:"亲和哈希",previousRequest:"上一个请求",nextRequest:"下一个请求",messages:"消息结构",requestSettings:"请求参数",instructions:"Instructions",tools:"工具",rawRequest:"原始请求",
    ...
    <SidebarInset>
      ...
      <div className="flex flex-1 flex-col gap-5 p-4 md:p-6">
        ...
        {userError ? <ErrorState message={message(userError)} /> : userLoading || !user ? <LoadingTable /> : <Routes>
          ...
          <Route path="/channels" element={<Channels sdk={sdk} locale={locale} />} />
          <Route path="/keys" element={<Keys sdk={sdk} locale={locale} />} />
...
function Channels({ sdk, locale }: { sdk: AuthSdk; locale: Locale }) {
  ...
  const { data: usageData, error: usageError } = useApiQuery<ChannelUsageResponse>(sdk, "/api/channels/usage")
  useEffect(() => () => { tokenRequest.current?.abort(); testRequest.current?.abort() }, [])
  function refreshChannels() { void queryClient.invalidateQueries({ queryKey: ["/api/channels"] }) }
  async function mutate(id: string, body: object) { try { await api(sdk, `/api/channels/${id}`, { method: "PATCH", body: JSON.stringify(body) }); refreshChannels(); toast.success(t.channelUpdated) } catch (cause) { toast.error(message(cause, t)) } }
  ...
  async function remove() {
    ...
    try { await api(sdk, `/api/channels/${deleteTarget.id}`, { method: "DELETE" }); setDeleteTarget(null); refreshChannels(); toast.success(t.channelDeleted) }
  ...
  return <><Card><CardHeader className="flex-row items-start justify-between"><div><CardTitle>{t.channelPool}</CardTitle><CardDescription>{t.channelDescription}</CardDescription></div><Button onClick={() => setOpen(true)}><PlusIcon data-icon="inline-start" />{t.addChannel}</Button></CardHeader><CardContent>
    ...
    <ChannelDialog sdk={sdk} locale={locale} open={open} onOpenChange={setOpen} oauth={oauth} setOauth={setOauth} onDone={() => { setOpen(false); refreshChannels() }} />
    <ChannelTokensDialog sdk={sdk} locale={locale} state={tokenDialog} onClose={closeTokens} onSaved={() => { closeTokens(); refreshChannels() }} />
...
function Keys({ sdk, locale }: { sdk: AuthSdk; locale: Locale }) {
  ...
  function refreshKeys() { void queryClient.invalidateQueries({ queryKey: ["/api/keys"] }) }
  async function create(event: FormEvent) { event.preventDefault(); try { const value = await api<{ secret: string }>(sdk, "/api/keys", { method: "POST", body: JSON.stringify({ name }) }); setSecret(value.secret); setName(""); refreshKeys() } catch (cause) { toast.error(message(cause, t)) } }
  async function revoke() { if (!revokeId) return; try { await api(sdk, `/api/keys/${revokeId}`, { method: "DELETE" }); refreshKeys(); setRevokeId(null); toast.success(t.keyRevoked) } catch (cause) { toast.error(message(cause, t)) } }
  if (loading) return <LoadingTable />; if (error) return <ErrorState message={error} />
...
function pageForPath(pathname: string): Page {
  ...
  return ({ "/dashboard": "dashboard", "/channels": "channels", "/keys": "keys", "/usage": "usage", "/audit": "audit", "/users": "users", "/settings": "settings" } as const)[pathname] ?? "dashboard"
}
function pageTitle(page: Page, locale: Locale) { const t = copy[locale]; return page === "request-detail" ? t.requestDetail : t[page] }
function pageDescription(page: Page, locale: Locale) { const t = copy[locale]; return ({dashboard:t.pageDashboard,channels:t.pageChannels,keys:t.pageKeys,usage:t.pageUsage,audit:t.pageAudit,"request-detail":t.pageRequestDetail,users:t.pageUsers,settings:t.pageSettings})[page] }
