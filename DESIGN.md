...
**Label/Mono Font:** 数据标识、哈希、API Key 前缀与请求 ID 使用系统等宽字体栈。
...
- **Mono Data**（400，0.75rem，1.5）：请求 ID、渠道 ID、模型名、限流头与脱敏凭据；长值允许复制并按场景换行或截断。
...
- **State:** 渠道状态同时显示图标或文字：可用、接近限流、冷却中、禁用、恢复检查中。
...
审计行固定展示时间、租户、API Key 前缀、渠道、能力/模型、状态、耗时、用量与请求 ID。原始详情在 Sheet 中按字段分组展示；敏感头与 OAuth 凭据必须脱敏，复制操作逐项授权并留下管理审计记录。
...
- **Do** 把渠道健康、限流余量、冷却截止时间、禁用原因和恢复条件放在同一可扫描上下文中。
- **Do** 用表格承载 API Key 用量和逐次调用审计，并提供时间、租户、Key、渠道、能力与结果筛选。
...
- **Don't** 在 UI、日志或复制操作中暴露完整 access_key、refresh_key 或租户 API Key。
